	...
	while (state == 'y' || state == 'Y')
	{
		...
		while (fread(&item, sizeof(item), 1, fp) == 1)
		{
			if (item.id == id)
			{
				...
				{
					fseek(temp_ptr, (flag1 - 1) * sizeof(item), 0);
				}
...
void Search_for_item()
{
	...
	while (fread(&item, sizeof(item), 1, fp) == 1)
	{
		...
		if(item.id!=id && fread(&item, sizeof(item), 1, fp) == 0 )
		{
...
void Item_Purchase()
{
	...
	{	//printf("this\n");
		...
		if (Item_wanted == item.id)
		{
			...
			printf("%d", item.price);
			printf("\n");
			printf("\n\t\t\t\t\t\tYOUR TOTAL BILL: %d", item.price);
			printf("\nPress \"1\" to return to main screen, \"2\"To return to main menu \"3\" to quit the program! ");
			char status;
			status = getch();
			if (status == '1')
			{
			...
			else
			{
				...
				puts("\n\n\n\n\n\n\t\t\t\tTHANK YOU");
				fflush(stdin);
				puts("\n\t\tFOR CHOOSING THE SOFTWARE SOLUTIONS");
				fclose(fp);
				getch();
				exit(0);
			}
		}
		else if (Item_wanted == 0)
		{
			main();
		}
		else if (Item_wanted != item.id && i==j )
		{	
			printf("Item does not exist in the database. Please enter a valid id.\nReloading Program......");
			sleep(2);
			Item_Purchase();
		}
	}
