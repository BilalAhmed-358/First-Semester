...
int main()
{
    ...
    struct P_and_Q points; struct P_and_Q *ptr_points=&points;
    ...
    for (i = 0; i < n; i++)
    {  
        *ptr_d+=sqrt(pow((ptr_points->Q[i]-ptr_points->P[i]),2));
    }
    printf("The value of Euclidian distance of the given point is %.3f\n", *ptr_d);
    getchar();
}